...
A live trading bot for Deriv's synthetic index digit contracts (DIGITEVEN /
DIGITODD), built from the Astra master spec with three deliberate scope
...
### The backtest simulator's one historical limitation (now moot)

`backtest/simulator.py`'s synthetic contract quote uses one flat payout
ratio. This used to be a real caveat back when Astra traded DIGITOVER/
DIGITUNDER: real Deriv payouts are priced per barrier (a barrier of 2 has a
much higher win probability, hence lower payout, than a barrier of 8), so a
backtest run could look "profitable" purely from a barrier's own built-in
win-rate geometry against an unrealistic flat payout assumption -- not a
real edge. Since the move to DIGITEVEN/DIGITODD (see below), both sides are
an exact 50/50 split with no barrier-dependent base rate to misrepresent, so
a flat assumed payout is a reasonable approximation. Still: real Deriv
payouts fluctuate with market conditions, so use the backtester to
sanity-check that the pipeline's plumbing behaves correctly (does it abstain
on noise? does it fire and win on an injected bias?), not to estimate real
returns precisely. Live trading is unaffected either way --
`execution/orders.py` and `pricing/payout.py` always fetch a real proposal
from Deriv immediately before every decision and again before every buy.

### Switched from Over/Under to Even/Odd

Astra now trades DIGITEVEN/DIGITODD exclusively -- DIGITOVER/DIGITUNDER
support has been fully removed, not just deprioritized. Two reasons drove
this:

1. **Over 2 / Under 7 had a 70% inherent win rate baked into the barrier's
   own geometry**, independent of any real predictive skill (7 of 10 digits
   satisfy `digit > 2`). That's exactly the confound the backtest
   limitation above used to describe -- it's easy to mistake "the barrier's
   own arithmetic" for "the model found an edge." Even/Odd is an exact
   50/50 split, so any edge Astra finds there is a much more honest signal
   of genuine skill.
2. It matches the pattern already used by a sibling bot in this account
   (`digit_evenodd_bot`).

What changed, mechanically: `models/ensemble.py::model_agreement()` now
splits digits into `EVEN_DIGITS`/`ODD_DIGITS` instead of an over/under
barrier split; `decision/decision_engine.py`'s `Decision` and
`ArchitectureDecision` fields are `even_probability`/`odd_probability`,
`even_edge`/`odd_edge`, `even_ev`/`odd_ev` (decisions are `TRADE_EVEN` /
`TRADE_ODD`, no barrier suffix); `ingestion/deriv_client.py` and
`pricing/payout.py::ContractQuote` now treat `barrier` as optional, since
DIGITEVEN/DIGITODD contracts don't take one at all (sending one is
rejected by Deriv's API). The underlying digit-probability machinery --
Global's 8 shared models, Specialist's 10 per-digit binary specialists,
Hybrid's blend, and the whole architecture competition -- didn't need to
change conceptually, since all three still estimate the full 10-digit
distribution regardless of which contract type trades on top of it; only
the "which digits count as which side" logic and the contract-layer plumbing
changed.

`database/schema.sql` has an additive migration for existing deployments:
new `even_*`/`odd_*` columns are added to `astra_predictions` and
`astra_trades.barrier` is relaxed to nullable, without touching or renaming
the old `over_*`/`under_*` columns -- those stay in place holding
historical Over/Under data as an archival record. Re-run `schema.sql`
against an existing Supabase project before redeploying.
...
`ASTRA_SYMBOLS` now defaults to `R_100,1HZ100V` instead of being blank.
...
### A second deployment log caught a regime-detection bug

A later run on `1HZ10V` showed zero trades across ~2,200 ticks (~37
minutes), with the new tick-summary logging (below) pointing squarely at
`unstable_regime:MODEL_DISAGREEMENT` firing on essentially every tick, even
though the architecture competition had already promoted `specialist` as
champion. The cause: `DecisionEngine.evaluate()`'s regime detection was
hardcoded to always compute model agreement from Global's 8 shared models,
regardless of which architecture was actually champion. So Specialist could
be -- and was -- stuck in a permanently "unstable" regime purely because
Global's unrelated models disagreed with each other, even on ticks where
Specialist's own two internal signals agreed just fine. Every other gate
(the actual trade-quality checks in `evaluate_architecture_decision`) was
already correctly using each architecture's own agreement; this was the one
place still reading a fixed source. Fixed to read
`snapshot.agreement_inputs[competition.champion]` instead, and covered by
`tests/test_architecture_competition.py::test_regime_detection_uses_the_champions_own_agreement_not_always_global`,
which constructs a case where Global and Specialist disagree about how much
they agree with themselves and confirms the regime-relevant number now
actually tracks whichever architecture is champion. Note switching to
Even/Odd (above) does NOT fix this by itself -- it's an unrelated,
