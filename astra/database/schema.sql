...
create table if not exists astra_predictions (
    ...
    even_probability double precision,
    odd_probability double precision,
    even_edge double precision,
    odd_edge double precision,
    even_ev double precision,
    odd_ev double precision,
    regime text,
...
create table if not exists astra_trades (
    ...
    symbol text not null,
    ts timestamptz not null default now(),
    contract_type text not null,           -- DIGITEVEN | DIGITODD
...
-- Migration: Astra switched from DIGITOVER/DIGITUNDER to DIGITEVEN/DIGITODD
-- contracts. For a deployment that already ran the old schema (with
-- over_probability/under_probability/etc columns and a NOT NULL barrier on
-- astra_trades): add the new columns additively and relax the barrier
-- constraint, rather than destructively renaming -- the old over_*/under_*
-- columns are left in place holding historical Over/Under data as an
-- archival record. New rows only populate the even_*/odd_* columns. Safe to
-- re-run.
alter table astra_predictions add column if not exists even_probability double precision;
alter table astra_predictions add column if not exists odd_probability double precision;
alter table astra_predictions add column if not exists even_edge double precision;
alter table astra_predictions add column if not exists odd_edge double precision;
alter table astra_predictions add column if not exists even_ev double precision;
alter table astra_predictions add column if not exists odd_ev double precision;
