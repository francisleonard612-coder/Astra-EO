...
from models.ensemble import EVEN_DIGITS, ODD_DIGITS, combine, model_agreement
...
class Decision:
    ...
    decision: str  # TRADE_EVEN | TRADE_ODD | NO_TRADE
    reason: str
    sample_size: int
...
class ArchitectureDecision:
    ...
    calibrated_even: float
    calibrated_odd: float
    even_edge_result: Any
    odd_edge_result: Any
...
def evaluate_architecture_decision(
    *, ensemble_vec: np.ndarray, predictions_for_agreement: dict[str, np.ndarray],
    ...
    calibrated_even = calibration_even.calibrate(raw_even)
    calibrated_odd = calibration_odd.calibrate(raw_odd)

    even_edge_result = compute_edge(quote_even, calibrated_even) if quote_even else None
    odd_edge_result = compute_edge(quote_odd, calibrated_odd) if quote_odd else None

    candidates: list[tuple[str, Any, float, dict]] = []
    for side, edge_result, cal_tracker, agreement_key in (
        ("EVEN", even_edge_result, calibration_even, "even_agreement"),
        ("ODD", odd_edge_result, calibration_odd, "odd_agreement"),
    ):
    ...
    return ArchitectureDecision(
        ...
        even_edge_result=even_edge_result, odd_edge_result=odd_edge_result,
...
class SymbolPipeline:
    ...
    def __init__(self, symbol: str, cfg: AstraConfig):
        ...
        self.calibration_even = CalibrationTracker(
        ...
        self.calibration_odd = CalibrationTracker(
            method=cal_cfg.get("method", "isotonic"),
            min_samples=cal_cfg.get("min_samples_to_calibrate", 200),
            refit_every=cal_cfg.get("refit_every", 100),
        )
        ...
        self._pending_even_prob: float | None = None
        self._pending_odd_prob: float | None = None
    ...
    def observe(self, state: SymbolState, bundle, predictions: dict[str, np.ndarray], actual_digit: int) -> None:
        for name, model in self.registry.models.items():
            try:
        ...
        if self._pending_even_prob is not None:
            outcome_even = 1 if actual_digit % 2 == 0 else 0
            self.calibration_even.record(self._pending_even_prob, outcome_even)
        if self._pending_odd_prob is not None:
            outcome_odd = 1 if actual_digit % 2 == 1 else 0
            self.calibration_odd.record(self._pending_odd_prob, outcome_odd)
...
class DecisionEngine:
    ...
    async def evaluate(self, client: DerivClient, state: SymbolState, competition: "ArchitectureCompetitionManager",
        ...
        if not state.has_min_samples(self.min_samples):
            return Decision(
                timestamp=now, symbol=symbol, probabilities={}, even_probability=0.0, odd_probability=0.0,
                even_edge=None, odd_edge=None, even_ev=None, odd_ev=None, regime="UNKNOWN",
        ...
        # Regime detection must reflect the CHAMPION architecture's own
        # internal agreement, not always Global's -- using Global's 8-model
        # agreement regardless of which architecture is actually champion
        # was a real bug caught from a live deployment log: it meant
        # Specialist (or Hybrid) could be stuck permanently in
        # MODEL_DISAGREEMENT regime -- and therefore blocked from trading
        # entirely -- because GLOBAL's models disagreed with each other,
        # even on ticks where Specialist's own two internal signals agreed
        # just fine. Each architecture's internal agreement was already being
        ...
        quote_even = await get_quote(
            client, symbol, "DIGITEVEN", stake, self.duration, self.duration_unit, currency,
        )
        quote_odd = await get_quote(
            client, symbol, "DIGITODD", stake, self.duration, self.duration_unit, currency,
        )
        snapshot.quote_even = quote_even
        snapshot.quote_odd = quote_odd
        ...
        champion_decision = evaluate_architecture_decision(
            ...
            calibration_even=calibration["even"], calibration_odd=calibration["odd"],
        ...
        for arch in ARCHITECTURES:
            ...
            snapshot.decisions[arch] = evaluate_architecture_decision(
                ...
                calibration_even=cal["even"], calibration_odd=cal["odd"],
                regime=regime, quote_even=quote_even, quote_odd=quote_odd,
                sample_size=state.total_observed,
                mp_cfg=self.mp_cfg, risk_ok=risk_ok, risk_reason=risk_reason,
            )
        ...
        return Decision(
            ...
            even_probability=champion_decision.calibrated_even,
            odd_probability=champion_decision.calibrated_odd,
            even_edge=champion_decision.even_edge_result.edge if champion_decision.even_edge_result else None,
            odd_edge=champion_decision.odd_edge_result.edge if champion_decision.odd_edge_result else None,
            even_ev=champion_decision.even_edge_result.expected_value if champion_decision.even_edge_result else None,
            odd_ev=champion_decision.odd_edge_result.expected_value if champion_decision.odd_edge_result else None,
            regime=regime,
            model_agreement=champion_decision.agreement,
            calibration_quality={
                "even": calibration["even"].quality_score(),
                "odd": calibration["odd"].quality_score(),
            },
