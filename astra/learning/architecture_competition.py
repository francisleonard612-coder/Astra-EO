...
class ArchitectureCompetitionManager:
    def __init__(self, symbol: str, cfg: AstraConfig, repository=None):
        ...
        self.calibration: dict[str, dict[str, CalibrationTracker]] = {
            arch: {
                "even": CalibrationTracker(method=cal_cfg.get("method", "isotonic"),
                                            min_samples=cal_cfg.get("min_samples_to_calibrate", 200),
                                            refit_every=cal_cfg.get("refit_every", 100)),
                "odd": CalibrationTracker(method=cal_cfg.get("method", "isotonic"),
                                           min_samples=cal_cfg.get("min_samples_to_calibrate", 200),
                                           refit_every=cal_cfg.get("refit_every", 100)),
            }
    ...
    def observe_pending(self, state: SymbolState, actual_digit: int) -> None:
        ...
        self.global_pipeline.observe(state, pending.bundle, pending.global_predictions, actual_digit)
        ...
        for arch in ARCHITECTURES:
            ...
            self.calibration[arch]["even"].record(decision.calibrated_even, outcome_even)
            self.calibration[arch]["odd"].record(decision.calibrated_odd, outcome_odd)

            best_ev = None
            for er in (decision.even_edge_result, decision.odd_edge_result):
            ...
            if decision.side is not None and decision.chosen_edge_result is not None:
                won = (outcome_even == 1) if decision.side == "EVEN" else (outcome_odd == 1)
    ...
    def _raw_metrics(self) -> dict[str, dict[str, float]]:
        ...
        for arch in ARCHITECTURES:
            ...
            s["calibration"] = (cal["even"].quality_score() + cal["odd"].quality_score()) / 2.0
            raw[arch] = s
        return raw
